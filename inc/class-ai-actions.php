...
class WritgoCMS_AI_Actions {
	...
	private function __construct() {
		...
		add_action( 'wp_ajax_writgocms_ai_rewrite', array( $this, 'ajax_rewrite_text' ) );
		add_action( 'wp_ajax_writgocms_ai_improve', array( $this, 'ajax_improve_text' ) );
		add_action( 'wp_ajax_writgocms_ai_expand', array( $this, 'ajax_expand_text' ) );
		add_action( 'wp_ajax_writgocms_ai_shorten', array( $this, 'ajax_shorten_text' ) );
		add_action( 'wp_ajax_writgocms_ai_add_links', array( $this, 'ajax_add_links' ) );
		add_action( 'wp_ajax_writgocms_ai_rewrite_block', array( $this, 'ajax_rewrite_block' ) );
		add_action( 'wp_ajax_writgocms_ai_rewrite_article', array( $this, 'ajax_rewrite_article' ) );
		add_action( 'wp_ajax_writgocms_ai_seo_optimize', array( $this, 'ajax_seo_optimize' ) );
		add_action( 'wp_ajax_writgocms_ai_generate_meta', array( $this, 'ajax_generate_meta' ) );
		add_action( 'wp_ajax_writgocms_ai_get_usage', array( $this, 'ajax_get_usage' ) );
	}
	...
	private function get_provider() {
		if ( null === $this->aiml_provider ) {
			$this->aiml_provider = WritgoCMS_AIML_Provider::get_instance();
		}
	...
	private function check_permissions() {
		if ( ! current_user_can( 'edit_posts' ) ) {
			return new WP_Error( 'permission_denied', __( 'Je hebt geen toestemming om deze actie uit te voeren.', 'writgocms' ) );
		}
		...
		if ( class_exists( 'WritgoCMS_License_Manager' ) ) {
			...
			if ( ! $license_manager->is_license_valid() ) {
				return new WP_Error( 'license_invalid', __( 'Je licentie is niet actief. Activeer je licentie om WritgoAI te gebruiken.', 'writgocms' ) );
			}
		}
	...
	public function ajax_rewrite_text() {
		check_ajax_referer( 'writgocms_ai_toolbar_nonce', 'nonce' );
		...
		if ( empty( $text ) ) {
			wp_send_json_error( array( 'message' => __( 'Geen tekst geselecteerd.', 'writgocms' ) ) );
		}
	...
	public function ajax_add_links() {
		...
		if ( empty( $existing_posts ) ) {
			wp_send_json_error( array( 'message' => __( 'Geen geschikte interne links gevonden.', 'writgocms' ) ) );
		}
...
Geef de tekst terug met toegevoegde interne links. Als er geen relevante links zijn, geef dan de originele tekst terug.',
	...
	public function ajax_rewrite_block() {
		...
		if ( empty( $content ) ) {
			wp_send_json_error( array( 'message' => __( 'Geen blok inhoud gevonden.', 'writgocms' ) ) );
		}
	...
	public function ajax_rewrite_article() {
		...
		if ( empty( $content ) ) {
			wp_send_json_error( array( 'message' => __( 'Geen artikel inhoud gevonden.', 'writgocms' ) ) );
		}
  ...
  "seo_score": 75
	...
	public function ajax_generate_meta() {
		...
		if ( empty( $content ) && empty( $title ) ) {
			wp_send_json_error( array( 'message' => __( 'Geen inhoud gevonden.', 'writgocms' ) ) );
		}
...
// Initialize.
WritgoCMS_AI_Actions::get_instance();
